  ...
  <div class="drwarpper">
    <div class="consoleclazz">
      <el-row>
        ...
        <el-col :span="2">
          <el-icon class="box-icon"><Plus /></el-icon>
        </el-col>
        <el-col :span="6">
          ...
          <div class="boxs">
            <div class="box-icon">
              <template v-if="result[0]">
                <img :src="'/png' + result[0].p.icon" class="box-img" />
              </template>
            </div>
          </div>
        </el-col>
    ...
    <el-row class="mrtop">
      <span>此模拟转移会消耗红框装备</span>
    </el-row>
    <el-row class="mrtop">
      <el-button type="primary" round @click="eat">吃</el-button>
    </el-row>
  </div>
</template>
  ...
  computed: {
    ...
    result() {
      if (this.leftSelectedEquip && this.rightSelectedEquip) {
        let r = this.leftSelectedEquip.eat(this.rightSelectedEquip);
        let EG = r[0];
        let cost = r[1];
        return [new Equip(EG, this.leftSelectedEquip.id), cost];
      }
      return [];
    },
  },
  methods: {
    ...mapActions(["addEquipToBag", "set", "eatEq", "addCost"]),
    changeLeft(val) {
      let eqleft = this.bag[val];
      let eqright = this.rightSelectedEquip;
      if (eqleft.p.name.indexOf("共鸣") > -1) {
        ElMessage({
          message: "不能选择共鸣装备作为成长装备!",
          type: "warning",
        });
    ...
    eat() {
      const eq = this.result[0];
      const cost = this.result[1];
      this.eatEq(eq);
      this.addCost(cost);
    },
  },
